...
const HourlyForecast: React.FC<HourlyForecastProps> = ({ processedHourly }) => {
  return (
    <section aria-labelledby="hourly-forecast-heading" className="bg-gray-700/50 p-6 rounded-xl shadow-lg">
      <h2 id="hourly-forecast-heading" className="text-2xl font-semibold mb-4 text-gray-300">پیش‌بینی ساعتی</h2>
      <div className="flex overflow-x-auto space-x-4 pb-4" dir="ltr">
        {processedHourly.map((hour, i) => {
          const weatherInfo = WMO_CODES[hour.weather_code] || { description: 'نامشخص', icon: '❓' };
          return (
            <div key={i} className="flex-shrink-0 text-center bg-gray-800/60 p-4 rounded-lg w-24">
              <p className="font-medium">{new Date(hour.time).toLocaleTimeString('fa-IR', { hour: '2-digit', minute: '2-digit' })}</p>
