...
const CurrentWeather: React.FC<CurrentWeatherProps> = ({ weatherData }) => {
  const weatherInfo = WMO_CODES[weatherData.current.weather_code] || { description: 'نامشخص', icon: '❓' };
  const cityName = weatherData.timezone.split('/')[1]?.replace('_', ' ') || 'مکان انتخاب شده';

  return (
    <section aria-labelledby="current-weather-heading" className="bg-gray-700/50 p-6 rounded-xl shadow-lg">
      <h2 id="current-weather-heading" className="text-2xl font-semibold mb-4 text-gray-300">آب و هوای فعلی در {cityName}</h2>
      <div className="flex flex-col sm:flex-row justify-between items-center">
        ...
        <div className="text-sm text-gray-400 space-y-2 text-center sm:text-right">
          <p>دمای احساسی: {Math.round(weatherData.current.apparent_temperature)}°</p>
          <p>رطوبت: {weatherData.current.relative_humidity_2m}%</p>
          <p>باد: {weatherData.current.wind_speed_10m} km/h</p>
        </div>
      </div>
    </section>
  );
